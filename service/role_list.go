...
func (s *RoleListService) CreatePermission(permission *model.Permission, role int) (*model.Permission, error) {
	result, err := s.repo.RoleList.CreatePermission(permission, role)
	if err != nil {
		return nil, err
	}
	return result, nil
